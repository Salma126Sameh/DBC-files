...
#include <math.h>
#include <stdlib.h>
#include "usbd_cdc_if.h" // Include CDC interface header
...
//			"AT+CFUN=1,1", // software resart the module
			"AT+CGNSPWR=1", "AT+CSQ",
			"AT+CIPSHUT", // //deAttach to GPRS
			"AT+CSTT=\"internet\"",
			"AT+CGATT=1", //Attach to GPRS
			"AT+CIICR", // Activate GPRS Context
			"AT+CIFSR", // Get IP
			"AT+CIPCLOSE", // to make sure the connection closed before starting a new one
			"AT+CIPMUX=0", "AT+CIPTKA=1,300,60,5",
			/*
			 * Parameters <n>:
			 * 0 Normal mode – when the server receives TCP data, it will responsd SEND OK
...
//	memset(rxBuffer, 0, MAX_RESPONSE_LEN);
//
//	if (HAL_UART_Receive(huart, rxBuffer, MAX_RESPONSE_LEN - 1, timeout)
//			== HAL_OK) {
//		DEBUG_PRINT("#GRN#Response after raw send: %s\n", rxBuffer);
//	} else {
//		DEBUG_PRINT("#RED#No response after raw data(or receive timeout)\n");
//	}
...
//	sendATCommand(huart, "AT+CSQ", response, UART_TIMEOUT);
	sendATCommand(huart, "AT+CIPSEND", response, UART_TIMEOUT);
//	HAL_Delay(200);
	sendMQTTPacket(huart, mqtt_packet, sizeof(mqtt_packet), 4000);
//	HAL_Delay(500);
}
